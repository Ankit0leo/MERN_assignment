...
const determineAgeGroup = (person) => {
...
console.log(determineAgeGroup(person1));
...
const totalItemsInStock = (inventory) => {
    return Object.values(inventory).reduce((total, count) => total + count, 0);
};
console.log(totalItemsInStock(inventory2));
...
const areAllPropertiesFilled = (details) => {
    return Object.values(details).every(value => value !== undefined && value !== null && value !== '');
};
console.log(areAllPropertiesFilled(details4));
...
console.log(averageProductPrice(products5).toFixed(2));
...
console.log(totalOrderCost(order9));
